...
mPFC contributes to goal-directed navigation mainly by representing task-relevant spatial context, destinations, and control variables when behavior must stay flexible, not by acting as a simple store of persistent spatial goals. The strongest causal evidence comes from mPFC-linked circuit effects on hippocampal prospective coding and replay-timed rule updating, while the strongest counterweight is that canonical goal-selective delay activity can disappear in flexible spatial working-memory tasks [[raw/summaries/ito2015_prefrontal_thalamic_hippocampus|Ito et al. 2015]] [[raw/summaries/denbakker2023_sharp_wave_prefrontal_rule|den Bakker et al. 2023]] [[raw/summaries/bohm2020_pfc_spatial_working_memory|Böhm et al. 2020]]. This page therefore treats mPFC as a broad region-level navigation and control node, leaving the narrower hippocampal-mPFC prospective-coding mechanism to its dedicated companion page.
