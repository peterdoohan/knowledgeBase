...
- d QUANTIFICATION AND STATISTICAL ANALYSIS 
   ...
   - B CA1-PFC population reactivation analysis 

   - B Model simulations for reactivation analysis B CA1-PFC activation ratio and pairwise reactivation B Statistical analysis 
...
## Dynamics of Awake Hippocampal-Prefrontal 

Replay for Spatial Learning 

and Memory-Guided Decision Making 

Justin D. Shin, Wenbo Tang, and Shantanu P. Jadhav 

**==> picture [540 x 586] intentionally omitted <==**

**Figure S1. (Related to Figure 1). Recording locations, cluster quality, and behavioral performance of all animals** 

**(A)** Recording locations for all 6 rats (color coded). The recording sites were reconstructed from the electrolytic lesions in _post hoc_ Nissl-stained coronal brain sections and mapped onto the stereotaxic atlas (Paxinos and Watson, 2004). The distance from Bregma (mm) is denoted on the right for each section. Electrodes were localized to target areas in dorsal area CA1 and medial prefrontal cortex (PFC) after histology (PFC electrodes 

1 

primarily in PreLimbic (PrL) cortex, with a few electrodes from one animal KL8 in Anterior Cingulate Cortex (ACC) area of PFC). 

**(B** and **C)** Representative histological sections through **(B)** PFC and **(C)** dorsal hippocampus. _Left_ : Coronal Nisslstained section illustrating lesion locations at end of tetrode tracks (marked by arrowheads). _Right_ : Schematic illustration of the section shown on the left based on the stereotaxic atlas (Paxinos and Watson, 2004) (colored dots represent the lesion locations on the section). 

**(D** and **E)** Cluster examples of 5 single units (color-coded) recorded on **(D)** one CA1 and **(E)** one PFC tetrode from the first and last behavioral sessions, respectively. Scatter plots show the peak-to-trough amplitudes of spike waveforms recorded on 2 of the 4 channels (each dot representing a single sampled spike; spikes associated with each isolated single unit are shown in a different color). Spike waveforms of the example cells on 4 channels are shown on the corresponding right panels (line: mean; shading: SD). 

**(F** and **G)** Cluster quality for all animals across 8 behavior sessions, for **(F)** CA1 place cells and **(G)** PFC cells. For the W-maze single-day learning paradigm, single-units were tracked continuously across interleaved run and rest sessions (W-maze run sessions: 17.9 ± 1.0 mins per session, 8 sessions per rat; interleaved rest sessions in rest box: 23.0 ± 4.9 mins per session, 9 sessions per rat; total recording duration: 6.04 ± 0.37 hours per rat; mean ± SD). **(Fi** and **Gi)** From _top_ to _bottom_ , isolation distance (Schmitzer-Torbert et al., 2005), cluster center-of-mass shift (Mallory et al., 2018) across sessions (spikes from the first session of the Animal ER1 were clustered separately and therefore excluded from the center-of-mass shift analysis), correlation coefficient of spike waveforms in two consecutive sessions (Li et al., 2017) (Fisher-transformed for normality). Each dot on the scatter plots represents an isolated single unit (All cells that were continuously tracked across all 8 behavioral sessions with stable spiking waveforms, and with firing fields on the W-track, are shown; CA1: _n_ = 216 cells, average of 36.0 ± 9.9 cells per animal in mean ± SD; PFC: _n_ = 154 cells, average of 25.7 ± 9.9 cells per animal). Dotted horizontal lines indicate inclusion thresholds used for each criterion. Error bars: mean ± 95% CI. Animal IDs and total number of cells simultaneously and continuously recorded across 8 sessions are denoted and color-coded on the top. All included units met at least one of the inclusion criteria. For the primary criteria of isolation distance and center-of-mass shift, 98.6% (213/216) CA1 and 98.1% (151/154) PFC cells met the thresholds across all sessions recorded. And 93.1% (201/216) CA1 and 94.8% (146/154) PFC units met all 3 inclusion criteria. **(Fii** and **Gii)** Isolation distance ( _left_ ) and center-of-mass shift ( _right_ ) of all the recorded cells from one animal (JS14) across sessions. Each dotted line represents one recorded cell. 

**(H)** Behavioral performance of all animals. Data are presented as mean ± 90% CIs for the outbound (blue; _left_ ) and inbound (teal; _right_ ) components from all the 6 animals that learned the W-maze task over 8 sessions in a single-day. Learning curves were estimated using a state-space model (Jadhav et al., 2012; Maharjan et al., 2018; Smith et al., 2004). Horizontal dashed lines: chance-level performance of 0.5. Note that the state-space model describes an animal’s choice behavior as an evolving process, and starts the estimation on the first trial from the baseline probability of 0.5 for both outbound and inbound components. The initial dip in performance for the inbound component due to perseveration is similar to previous reports (Jadhav et al., 2012; Kapellusch et al., 2018; Kim and Frank, 2009; Maharjan et al., 2018). Performance in the first and final sessions: 53.67 ± 0.02%, and 85.17 ± 0.06% for outbound component; 33.74 ± 15.40%, and 95.45 ± 0.04% for inbound component (data are presented as mean ± SD). 

2 

**==> picture [540 x 590] intentionally omitted <==**

## **Figure S2. (Related to Figures 1 and 2). Place-cell activity and additional illustrative replay events across different learning sessions** 

**(A-C)** Stability of place-cell activity. **(A)** Spatial stability of place fields increased over learning (**** _p_ < 1e-4, Friedman test), but reliable place-cell activity was apparent as early as the first sessions ( _p_ < 1e-3 compared to the chance by shuffling cell identity). The gray dashed lines indicate 95% confidence intervals of chance levels based on shuffling cell IDs. Note that stability was assessed as the correlation between the linearized maps for 

3 

all 4 trajectory types for two consecutive behavioral sessions (see **STAR Methods** ). **(B)** While place cells exhibited high spatial specificity during the first sessions (0.80 ± 0.14, mean ± SD), spatial specificity of place fields increased over learning (**** _p_ < 1e-4, Friedman test). Spatial specificity was computed as (1- _spatial sparsity_ ), where spatial sparsity is defined as the proportion of firing rate bins > 25% of the peak firing rate (Jadhav et al., 2016; Tang et al., 2017; Zielinski et al., 2019). **(C)** Most of the CA1 cells (159/216, 73.6%) exhibited typical place coding across all 8 sessions (peak rate ≥ 3 Hz), and the proportions of place cells did not change significantly from one session to another ( _p’s_ > 0.05, z-tests for proportions). Rows correspond to different sessions and columns correspond to CA1 cells. Colored entries in black denote instances when the neuron cannot be defined as a place cell in the given session (see **STAR Methods** ). 

**(D-L)** Additional examples of forward and reverse replay events of behavioral trajectories from different learning sessions. **(D-F)** Early session (Epochs 1-3) examples. **(G-I)** Middle session (Epochs 4-5) examples. **(J-L)** Late session (Epochs 6-8) examples. Data are presented as in **Figure 2** . **(i)** _Left_ : Place-cell activity during the SWR with ripple-filtered (150-250 Hz) LFP from one tetrode shown on top (black line). _Right_ : Corresponding linearized place fields on trajectories 1 to 4 sorted by their peak locations on the replay trajectory (red). **(ii)** Bayesian reconstruction of the decoded behavioral trajectories with the replay quality ( _r_ ) and _p_ -value based on timeshuffled data denoted on top. Cyan lines: the linear fit maximizing the likelihood along the replayed trajectory. Color bars: posterior probability. 

4 

**==> picture [520 x 654] intentionally omitted <==**

**Figure S3. (Related to Figure 3). Reverse and forward replay events during pauses between trajectories at reward wells** 

5 

**(A-H)** Representative replay events at reward wells during correct trials. 

**(A** and **B)** Example of center reward well transitions. **(A)** CA1 neural activity during a correct inbound-outbound working memory sequence (RUN1 to RUN2), with replay events at the center well. This center-well transition example consists of an inbound past trajectory (RUN1, left-to-center), followed by an outbound future trajectory (RUN2, center-to-right). Place-cell trajectory sequences ordered according to RUN2 and the alternative inbound past trajectory (RUN1-Alt) is shown. Raster plot for RUN1-Alt is shown from the nearest temporally adjacent trajectory in the past (see **STAR Methods** ). **(B)** All four replay events during the center-well transition, identified as forward replay of RUN2 (future taken path, 3 events) and reverse replay of RUN1-Alt (alternative past path, or not-taken past, one event). The behaviorally actualized path by the animal is denoted as the ‘taken’ path, and the alternative path is denoted as the ‘not-taken’ path. Data are presented as in **Figures 3A-3B** . 

**(C** and **D)** Example replay events at center well during early learning (sessions 1-3). **(E** and **F)** Example replay events at side wells during early learning. **(G)** Example replay events at center well during middle sessions (sessions 4-5). **(H)** Example replay events at the side well during late performance (sessions 6-8). Data are presented as in **Figures 3D** and **3E** . For each example, the behavioral sequence is shown in **(i)** and all the replay events during immobility at the well (i.e., current position; green circle) are shown in **(ii)** . _Top_ : Bayesian reconstruction of the replayed trajectory. Colored points overlaid on the replayed trajectory (black arrowhead line) indicate the Bayesian-decoded positions, with the color denoting relative time within the replay event (early in red, and late in yellow). _Bottom_ : ripple-filtered LFP and the sequential spiking of place cells during the SWR. 

**(I** and **J)** Distributions of **(I)** animals’ immobility times at reward wells, **(J)** the number of SWRs detected in immobility periods per choice (i.e., per transition) during incorrect trials originating from the center well. These incorrect (or error) trials occurred during learning of the outbound rule ( **Figure S1H** ). During these trials, animals were located at the center well after performing a correct past trajectory (inbound; rewarded), but were about to choose the next outbound trajectory incorrectly, as compared with correct trials in which future outbound trajectory was chosen correctly. Thus, both future correct and incorrect trials were preceded by the presence of reward at the center well, and the SWR replay events occurring at the center well _preceding_ a future correct or incorrect choice were used for the prediction analysis in **Figure 3K** , **Figures 5G-5H** and **Figures S6C-S6D** . The time spent at reward wells, number of SWRs per choice, and replay rate were similar for incorrect and correct trials ( _p_ = 0.68, 0.31 and 0.44, respectively; animal-by-animal rank-sum paired tests; replay rate: 0.097 ± 0.016 Hz for incorrect, 0.088 ± 0.034 Hz for correct as mean ± SD). The vertical dashed lines on the histograms in represent the mean values. 

**(K** and **L)** Distributions of **(K)** animals’ immobility times at reward wells, **(L)** the number of SWRs detected in immobility periods per choice (i.e., per transition) during unrewarded trials. Unrewarded trials were defined as trials that occurred at side wells upon the end of an incorrect outbound trajectory, and SWR events during unrewarded trials were thus defined as those occurring at the side wells after the completion of an incorrect path. The time spent at reward wells, number of SWRs per choice, and replay rate during unrewarded trials were significantly lower than those during correct trials ( _p’s_ = 0.031, animal-by-animal rank-sum paired tests; number of SWRs per trial: 1.54 ± 0.26 for unrewarded trials, 6.34 ± 2.56 for correct trials ending at the side wells; replay rate: 0.029 ± 0.018 Hz for unrewarded trials, 0.089 ± 0.029 Hz for correct trials ending at the side wells; data are presented as mean ± SD), consistent with previously reports (Singer et al., 2013; Singer and Frank, 2009). The vertical dashed lines on the histograms represent the mean values. 

**(M)** The bias in forward and reverse replay shown during correct trials ( **Figure 3F** ) was absent during unrewarded trials ( _p_ = 0.72 and 0.27 for forward and reverse replay, respectively; z-tests for proportions; _n.s._ , _p_ = 0.99, session-by-session rank-sum test). There were 29.8 ± 7.4 unrewarded trials per animal across all 8 behavioral sessions. During these unrewarded trials, there were in total 44.1 ± 17.9 SWR events and 5.7 ± 4.2 replay events per animal across all 8 behavioral sessions (data are presented as mean ± SD). 

6 

**==> picture [534 x 699] intentionally omitted <==**

7 

## **Figure S4. (Related to Figure 3). Control analyses examining the bias in forward and reverse replay of future and past choices** 

**(A)** The bias in reverse and forward replay of past and future choices is not affected by exclusion of bidirectional cells. Forward replay showed a significant bias for future choices, while reverse replay is biased for past choices, using unidirectional place cells only. Data are presented as in **Figures 3F** and **3G** . Unidirectional cells were defined as cells with a directionality index significantly higher than the shuffle surrogates in a given session (by randomly shuffling the trial labels of two running directions). See **Table S1** for the number of unidirectional cells in each session. 

**(B)** CA1 trajectory-dependent activity on the center stem of a W-maze, similar to previous reports of “splitter cells” (Ainge et al., 2007; Ferbinteanu and Shapiro, 2003; Frank et al., 2000; Ito et al., 2015; Wood et al., 2000). _Left_ : a representative example CA1 cell fired differentially during center-to-right vs. center-to-left outbound trials. _Right_ : a representative example CA1 cell fired differentially during right-to-center vs. left-to-center inbound trials. In each example, rate maps during each trial type are shown on the top panel, and means (solid lines) and SEMs (shadings) for firing rates during the first and last 20 trials on the maze (first and last trials on the experimental day) are shown on the middle and bottom panels, respectively (CP: choice point). For the first and last 20 outbound trajectories, 55/138 (39.9%) and 70/147 (47.6%) cells with place fields on the center stem respectively showed significant trajectory-dependent firing ( _p_ < 0.05, trial-label permutation test); and for the first and last 20 inbound trajectories, 59/167 (35.3%) and 99/146 (67.8%) cells with place fields on the central stem respectively showed significant trajectory-dependent firing. While about one-third of CA1 cells showed significant trajectorydependent firing during the early sessions, the number of splitter cells increased across sessions (early and late sessions: 1-3 and 7-8 respectively; 29.2% and 52.5% during early vs. late sessions, and _p_ < 1e-4 for outbound; and 38.3% and 70.9% during early vs. late sessions, and _p_ < 1e-4 for inbound; z-tests for proportions). 

**(C** and **D)** The bias in reverse and forward replay of past and future choices is not affected by exclusion of centerarm cells. We examined replay events with **(C)** at least two cells that have peak firing past the CP and outside the center arm (i.e., side-arm cells, 122.5 ± 6.4 out of 216 place cells, 57.0 ± 10.1%, mean ± SD across 8 sessions; Singer et al., 2013; Fernandez-Ruiz et al., 2019), as well as **(D)** events with side-arm cells only. Note that most of replay events had at least two side-arm cells active (98.0%, 792/808 reverse replay events; 96.6%, 657/680 forward replay events). 

**(E** and **F)** The bias in replay content for both reverse and forward replay was seen during the first (i.e., arrival; **E** ) as well as the second (i.e., departure; **F** ) half of time spent in reward wells. 

**(G-I)** The bias in replay content did not differ appreciably during engaged **(G)** vs. disengaged **(H)** periods (Ólafsdóttir et al., 2017), although **(I)** the replay rate was significantly higher during disengaged than engaged periods. For trials with an immobility duration (defined as ≤ 4 cm/s) at a reward well longer than 6 s, the total time spent at the well was equally divided into 4 parts; the first and last parts were defined as engaged periods, and the rest were considered as disengaged. In **(I)** , each dot pair represents a session from one animal. **** _p_ < 1e-4, *** _p_ < 0.001, rank-sum paired tests. Error bars: SEM. 

**(J)** Spatial distributions of the peak positions of place fields for the 4 trajectory types (schematic of these trajectories shown on top). The _x_ -axis shows the linearized positions of each trajectory with the start and end well IDs denoted (C: center well; L: left well; R: right well). We found an accumulation of place fields specifically at the center reward well (goal location; Grubb’s test for outliers, **** _p_ < 1e-4, * _p_ < 0.05), consistent with previous reports of over-representation of goal locations (Dupret et al., 2010; Pfeiffer and Foster, 2013). Prob = probability. 

**(K** and **L)** Spatial distributions of the decoded positions during replay at the 3 different reward wells (color-coded). Violin plots are shown in **(K)** , and histograms are shown in **(L)** . Note that consistent with the results in **Figure 3** , reverse replay is biased to past paths, whereas forward replay is biased to future paths. For example, at the center well, reverse replay is biased towards positions along the past trajectories of R-to-C and L-to-C (Hartigan’s 

8 

dip test for bimodality, dip = 0.062, _p_ < 1e-4); whereas forward replay is biased towards positions along the future trajectories of C-to-R and C-to-L (Hartigan’s dip test for bimodality, dip = 0.065, _p_ < 1e-4). In addition, consistent with previous studies (Davidson et al., 2009; Diba and Buzsáki, 2007; Foster and Wilson, 2006; Gomperts et al., 2015; Karlsson and Frank, 2009), we found that the probability of decoded spatial locations during replay, but specifically _only during reverse replay_ , was biased toward the animals’ current location at the corresponding reward well (current location is the only statistical outlier during replay at a given well; Grubb’s test for outliers, **** _p_ < 1e-4, *** _p_ < 0.001, * _p_ < 0.05; _p_ > 0.05 for forward replay shown in **Lii** ; see also **M** and **N** ). Note that the over-representation of current location in decoded positions for reverse replay cannot be explained solely by the spatial distributions of place fields shown in **(J)** , since only the center well has over-representation of place fields. 

**(M** and **N)** Distributions of start locations of replay trajectories (initiation bias) relative to the animal’s current position for **(M)** forward and **(N)** reverse replay at different reward wells. Consistent with previous studies (Davidson et al., 2009; Diba and Buzsáki, 2007; Foster and Wilson, 2006; Joo and Frank, 2018; Karlsson and Frank, 2009), we found that replay events, specifically only reverse replay events, tended to begin with the representations of the animal’s current position (current location is the only statistical outlier during reverse replay at a given well; Grubb’s test for outliers, **** _p_ < 1e-4; _p_ > 0.05 for forward replay). 

9 

**==> picture [528 x 458] intentionally omitted <==**

**Figure S5. (Related to Figure 4). Change in SWR replay properties over the course of learning** 

**(A)** SWR amplitude ( _left_ ) and frequency ( _right_ ) did not change significantly across 8 behavioral sessions ( _p_ = 

0.21 and _F_ (7, 35) = 1.90 for amplitude, _p_ = 0.31 and _F_ (7, 35) = 0.30 for frequency; repeated measures ANOVA). 

- **(B)** Immobility time at reward wells decreased with learning ( _p_ = 0.0018 and _F_ (6, 1241) = 3.531, one-way ANOVA; ** _p_ = 0.0063, Holm-Sidak's _post hoc_ tests). 

**(C)** Percentage of place cells active per replay event did not change significantly across sessions ( _n.s._ , _p_ = 0.069, Kruskal-Wallis test), but the percentage during non-significant ripples was significantly decreased across sessions ( _p_ < 0.0001, Kruskal-Wallis test; **** _p_ < 0.0001, Dunn’s _post hoc_ tests). 

**(D)** Candidate event rate significantly decreased across sessions ( _p_ < 0.0001 and _F_ (6, 30) = 19.98, repeated measures ANOVA; * _p_ = 0.022, Tukey’s _post hoc_ tests). Note that while SWR rate did not change significantly over learning ( **Figure 4B** ), place-cell spiking during SWRs significantly decreased across sessions, as shown in **C** , attributed to non-replay events, explaining the decrease in candidate event rate. 

10 

**(E)** Percentage of replay out of candidate events did not change across sessions ( _p_ = 0.12 and _F_ (6, 30) = 2.37, repeated measures ANOVA). In **A** , **D** and **E** , thin dashed lines are for individual animals, thick lines and error bars represent means and SEM. 

**(F)** Replay rate was not significantly different at the three reward wells ( _p_ = 0.16, Friedman test), and replay rate decreased across sessions for both center and side wells (Center well: _p_ = 0.0002, Friedman test; _p_ = 0.0038 for sessions 1-2 vs. 8, Dunn’s _post hoc_ tests; Side wells: _p_ = 0.0008, Friedman test; _p_ = 0.033 for sessions 1-2 vs. 7, _p_ = 0.012 for sessions 3 vs. 8, Dunn’s _post hoc_ tests; overall replay rate shown **Figure 4C** ). Each dot represents one session from a subject. Lines and error bars represent means and SEMs, respectively. 

**(G)** Percentage of either reverse or forward replay out of candidate events did not change across sessions ( _left_ ; _p_ = 0.53 and _F_ (6, 30) = 0.73 for forward replay, _p_ = 0.22 and _F_ (6, 30) = 1.67 for reverse replay; repeated measures ANOVA), but both forward and reverse replay rates decreased over learning ( _right_ ; _p_ = 0.016 and _F_ (6, 30) = 5.16 for forward replay, _p_ = 0.0035 and _F_ (6, 30) = 8.06 for reverse replay; repeated measures ANOVA with Tukey’s _post hoc_ tests). 

**(H** and **I)** Number of total SWRs per choice decreased across 8 behavioral sessions (* _p_ = 0.027, Kruskal-Wallis test with Dunn’s _post hoc_ ). 

**(J** and **K)** Number of total candidate events (or high-synchrony events, HSEs) per choice decreased across 8 behavioral sessions (**** _p_ < 1e-4, Kruskal-Wallis test with Dunn’s _post hoc_ ). 

**(L** and **M)** Number of total replay events per choice decreased across 8 behavioral sessions (**** _p_ < 1e-4, Kruskal-Wallis test with Dunn’s _post hoc_ ). In **H** , **J** and **L** , the vertical dashed lines on the histograms represent mean values. 
...
**Figure S6. (Related to Figures 5). Replay content with side-arm cells and during incorrect trials over learning** 

**(A** and **B)** Evolution of reverse and forward replay for events with side-arm cells during learning was similar to that shown in **Figures 5E** and **5F** . Only replay events with at least two side-arm cells active were shown. The observed trends were therefore not simply a result of potential ambiguous detection of center-arm cell firing on left vs. right trajectories. Note that although the number of splitter cells changed from early learning to late performance for both inbound and outbound trials ( **Figure S4B** ), an opposing learning gradient was observed only for replay events at side wells. Further, the distinct dynamics were still observed when we included only the replay events with at least two side-arm cells, confirming that changes in splitter cells are not the primary reason for the observed changes. 

**(C** and **D)** Taken paths cannot be predicted from the patterns of replay events during incorrect trials at the center well ( _n_ = 36, 27, and 18 trials, and _p_ = 0.16, 1, and 0.11 for reverse replay; _n_ = 34, 17, and 9 trials, and _p_ = 1, 0.55, and 1 for forward replay during early, middle and late sessions, respectively), similar to correct trials. Red horizontal lines on columns represent chance levels as in **Figures 5G** and **5H** . 

12 

**==> picture [540 x 625] intentionally omitted <==**

**Figure S7. (Related to Figures 6 and 7). Properties of PFC cells and CA1-PFC reactivation** 

**(A-B)** Spatial **(A)** stability and **(B)** specificity of PFC spatial maps increased over learning (**** _p’s_ < 1e-4, Friedman test), with reliable and spatially selective activity apparent from the first session ( _p_ < 0.0001 compared to the chance by shuffling cell identity in **A** ), similar to CA1 ( **Figures S2A** and **S2B** ). Stability and specificity are 13 

defined similarly as for CA1 (see **STAR Methods** ). The dashed lines indicate 95% confidence intervals of chance levels based on shuffling cell IDs. 

**(C)** PFC trajectory-dependent activity on the center stem of a W-maze. _Left_ : a representative example PFC cell fired differentially during center-to-right vs. center-to-left outbound trials. _Right_ : a representative example PFC cell fired differentially during right-to-center vs. left-to-center inbound trials. Data are presented as in **Figure S4B** for CA1 “splitter” cells. In agreement with previous reports of PFC “splitter” cells (Fujisawa et al., 2008; Ito et al., 2015), among all 154 PFC cells, 65 (42.2%) and 48 (31.7%) cells showed trajectory-dependent firing ( _p_ < 0.05, trial-label permutation test) on the center stem during the first and last 20 trials respectively for outbound trajectories, and 43 (27.9%) and 59 (38.3%) cells showed trajectory dependent firing during the first and last 20 trials respectively for inbound trajectories. This trajectory-dependent firing contributes to independent representation of left and right trajectories on the center stem, and the number of these PFC splitter cells remained relatively constant across sessions (40.3% and 40.3% in early vs. late sessions, and _p_ = 1 for outbound; 33.8% and 44.2% during early vs. late sessions, and _p_ = 0.06 for inbound; z-tests for proportions). 83.4 ± 11.9 out of 154 PFC cells are side-arm cells (50.2 ± 15.2%, mean ± SD across 8 sessions), and 89.1% (721/810) of CA1-PFC reactivation events had at-least 2 side-arm PFC cells active. 

**(D)** _Top_ : Percentage of PFC cells active per replay event did not change significantly across sessions ( _n.s._ , _p_ = 0.44, Kruskal-Wallis test), but the percentage during non-significant ripples significantly decreased across sessions ( _p_ < 0.0001, Kruskal-Wallis test; ** _p_ = 0.0019, Dunn’s _post hoc_ tests), similar to CA1 ( **Figure S5C** ). _Bottom_ : Firing rate of PFC cells during replay events did not change significantly across sessions ( _n.s._ , _p_ = 0.40, Kruskal-Wallis test). 

**(E)** _Left_ : Firing rate of PFC cells was not significantly different during replay of taken vs. not-taken trajectories ( _n.s._ , _p_ = 0.45 for reverse replay, and _p_ = 0.46 for forward replay; rank-sum tests). _Right_ : Percentage of PFC cells active was not significantly different during replay of taken vs. not-taken trajectories ( _n.s._ , _p_ = 0.46 for reverse replay, and _p_ = 0.68 for forward replay; rank-sum tests). Only replay during correct trials are shown. 

**(F)** CA1 replay quality of taken vs. not-taken trajectories during correct trials. Replay quality was measured as “ _R_ -squared” from the linear regression maximizing the likelihood along the replayed trajectory (see **STAR Methods** ). While the reactivation strength of CA1-PFC population was higher for taken vs. non-taken trajectory replay during correct trials ( **Figures 7F** and **7G** ), we found no significant difference in CA1 replay quality for taken vs. not-taken paths ( _p_ = 0.066, Kruskal-Wallis test). 

**(G)** Number of CA1-PFC reactivation events per SWR was higher during early vs. late sessions (first 4 vs. last 4), confirming our previous reports (Tang et al., 2017). 

**(H** and **I)** Change of CA1-PFC reactivation strength over learning. **(H)** Stronger CA1-PFC reactivation of CA1forward-replayed taken path compared to the not-taken path was observed only in late performance sessions ( _p_ = 0.21, 0.44, and 0.008 for CA1 replay of taken paths at early, middle, and late learning stages; _p_ = 0.53, 0.08, and 0.66 for CA1 replay of not-taken paths at early, middle. and late learning stages; rank-sum paired tests). **(I)** No statistically significant trend was observed for reverse CA1 replay at different learning stages ( _p_ = 0.77, 0.32, and 0.066 for CA1 replay of taken paths at early, middle and late learning stages; _p_ = 0.61, 0.33, and 0.57 for CA1 replay of not-taken paths at early, middle, and late learning stages; rank-sum paired tests), although a significant overall bias for reverse replay was present across all sessions ( **Figure 7G** ). Only correct trials were included. 

**(J** and **K)** Raster plots ( _top_ ) and corresponding PSTHs ( _bottom_ ) aligned to SWR synchronous events of four PFC **(J)** and two CA1 **(K)** neurons, showing activation during SWRs. Horizontal dashed lines: mean firing rates in the entire session. Lines and shadings in the PSTHs: means and SEMs. Activation ratio of each cell is denoted on the top of the corresponding panel. 

14 

**(L)** SWR co-firing as a function of spatial correlation for CA1-PFC ( _Left_ ) and CA1-CA1 ( _Right_ ) cell pairs for a high-performance animal (maximal performance level on the outbound: 89.21%; see also **Nii** ). Spatial correlations are divided into six subgroups with equal number of cell pairs (i.e., sextiles), similar to a previous study (Tang et al., 2017). Note the strong correlation between SWR cofiring vs. spatial correlation of CA1-PFC pairs ( _r_ = 0.11, _p_ = 2.3e-4), as well as CA1-CA1 pairs ( _r_ = 0.56, _p_ < 1e-10). 

**(M)** Animals’ maximal performance level on the outbound, spatial working memory component, is negatively correlated with their time spent at reward wells. Animals’ maximal performance level was measured as the highest performance reached on the outbound learning curve ( **Figure S1H** ). Error bars represent mean and SEM from 8 sessions for each animal. 

**(N)** Relationship between animals’ maximal performance level on the outbound task and **(Ni)** activation ratio of CA1 ( _Top_ ) and PFC ( _Bottom_ ) cells during SWR synchronous events, **(Nii)** reactivation strength of CA1-CA1 ( _Top_ ) and CA1-PFC ( _Bottom_ ) cell pairs, and **(Niii)** reactivation strength of CA1 ( _Top_ ) and CA1-PFC ( _Bottom_ ) populations (see **STAR Methods** ). Behavioral performance showed significant positive correlation with PFC activation ratio, CA1-PFC pairwise, and ensemble reactivation strength, but showed negative correlations with CA1-only parameters. Each dot with error bars represents mean with SEMs from 8 sessions of each animal. 

The observed positive correlation with CA1-PFC coherent reactivation is suggestive of a role in memory performance, but does not imply any direct links. The negative correlation with CA1 reactivation is surprising, and further corresponds to a negative correlation between total immobility time and memory performance (panel **M** ). We speculate that longer immobility times will lead to longer periods of disengagement from the task (Ólafsdóttir et al., 2017), and therefore reduced performance  ( **Figure S4I** shows replay rates during engaged and disengaged periods). Longer immobility times were still seen to be associated with stronger CA1 reactivation, possibly because CA1 reactivation is known to be influenced by pre-existing network structures (Dragoi and Tonegawa, 2011, 2014; Farooq et al., 2019), which can presumably maintain coherent structure in disengaged periods. On the other hand, longer immobility times were associated with weaker CA1-PFC reactivation, suggesting that animals’ engagement in the task (Ólafsdóttir et al., 2017) may influence whether or not CA1-PFC reactivation represents ongoing behavioral trajectories. 

15 

**==> picture [442 x 684] intentionally omitted <==**

16 

## **Figure S8. (Related to Figure 7). Template matching to detect CA1-PFC reactivation: method summary and comparison** 

**(A)** Generation of simulated data from a simple toy model. The simulated CA1-PFC neuronal population consists of 5 CA1 and 5 PFC cells with a many-to-one connectivity matrix. **(Ai)** Connectivity matrix (red for CA1-PFC pairs with synchronous firing; white for non-synchronous pairs). **(Aii)** Spatial maps of the simulated neurons on one trajectory with their peak firing rates denoted on the right (i.e., MAX FR). PFC neurons have multi-peaked spatial maps. **(Aiii)** Simulated spike trains during a SWR event. These spike trains were generated from a Poisson process, whose rate was determined by the spatial maps in **(Aii)** . 

**(B)** Template matching decoding of coherent CA1-PFC replay. **(Bi)** Synchronization matrix of CA1-PFC population activity calculated during running (RUN), with each element representing the correlation of spatial maps of a CA1-PFC cell pair. **(Bii)** Similarly, a synchronization matrix for SWR was measured based on the pairwise correlation of spike trains during the SWR. **(Biii)** The reactivation strength was then computed as the correlation coefficient ( _r_ ) between the synchronization matrices during RUN vs. SWR (each dot representing a CA1-PFC pair, red line from linear regression). For the simulated population, the reactivation strength is 0.49, significantly higher than its shuffle surrogates (randomly shuffling spike times during the SWR; _p_ = 0.004; note that the SWR spike trains of the simulated data are a sample of the Poisson process during RUN). **(Biv)** Detailed view of the CA1-PFC coordination using example cell pairs indicated by stars on the connectivity matrix in **(Ai)** . Curves: spatial maps. Ticks: spikes during the SWR. 

**(C** and **D)** Comparison of template matching, Bayesian decoding (O'Neill et al., 2017), and line-fitting (Ólafsdóttir et al., 2016, 2017) using the model in **(A)** . Despite strong correlation of the population activity during RUN and the SWR (synchronization detected using template matching in **B** ), the PFC activity during the SWR was detected **(C)** as a “non-replay” event by Bayesian decoding due to a lack of sequential structure ( _p_ = 0.29 by comparison to time shuffles), and **(D)** not coherent with the replay in CA1 using line-fitting ( _p_ = 0.06 by comparison to time shuffles). 

**(E** and **F)** Bayesian decoding and line-fitting results of the two reactivation events shown in **Figures 7A** and **7B** , respectively. **(E)** Note that for the event in **Figure 7A** with a clear sequential structure, the line-fitting method also detected a coherent CA1-PFC reactivation ( _p_ = 0.001), but **(F)** not for the event in **Figure 7B** with multipeaked, many-to-one coordination of CA1-PFC activity ( _p_ = 0.30 by comparison to time shuffles). White lines on Bayesian-reconstruction matrices for Bayesian decoding represent the linear fit maximizing the likelihood along the trajectory, and for line-fitting mark the extent of the line-fit based on CA1 place-cell activity (± 15 cm). 

17 

|**Animal ID**|**CA1 place**<br>**cells**|**Session no.**|**Unidirectional cells (percentage)**|**Mean ± SD**<br>**(% unidirectional)**|
|---|---|---|---|---|
|**ER1**|44|1<br>2<br>3<br>4<br>5<br>6<br>7<br>8|14 (31.8%)<br>19 (43.1%)<br>32 (72.7%)<br>32 (72.7%)<br>28 (63.6%)<br>33 (75.0%)<br>30 (68.2%)<br>30 (68.2%)|61.9 ± 15.8|
|**KL8**|47|1<br>2<br>3<br>4<br>5<br>6<br>7<br>8|13 (27.7%)<br>9 (19.1%)<br>21 (44.7%)<br>28 (59.6%)<br>29 (61.7%)<br>28 (59.6%)<br>34 (72.3%)<br>27 (57.4%)|50.3 ± 18.3|
|**JS14**|37|1<br>2<br>3<br>4<br>5<br>6<br>7<br>8|14 (37.8%)<br>19 (51.4%)<br>29 (78.4%)<br>30 (81.1%)<br>32 (86.5%)<br>30 (81.1%)<br>31 (83.8%)<br>31 (83.8%)|73.0 ± 18.0|
|**JS15**|40|1<br>2<br>3<br>4<br>5<br>6<br>7<br>8|7 (17.5%)<br>29 (72.5%)<br>23 (57.5%)<br>24 (60.0%)<br>29 (72.5%)<br>29 (72.5%)<br>27 (67.5%)<br>29 (72.5%)|61.6 ± 18.8|
|**JS17**|24|1<br>2<br>3<br>4<br>5<br>6<br>7<br>8|8 (33.3%)<br>12 (50.0%)<br>19 (79.2%)<br>16 (66.7%)<br>19 (79.2%)<br>17 (70.8%)<br>17 (70.8%)<br>14 (58.3%)|63.5 ± 15.7|
|**JS21**|24|1<br>2<br>3<br>4<br>5<br>6<br>7<br>8|11 (45.8%)<br>12 (50.2%)<br>14 (58.3%)<br>18 (75.0%)<br>18 (75.0%)<br>18 (75.0%)<br>18 (75.0%)<br>13 (54.2%)|63.5 ± 12.7|
|**Total**|216|-|133 (61.6%)|61.6 ± 15.0|



## **Table S1. (Related to Figure 1). Summary of unidirectional cells** 

Number of CA1 place cells recorded continuously over 8 learning sessions, the number and percentage of unidirectional cells in each session, and percentage of unidirectional cells (mean ± SD) for each animal are shown. 

18 

|**Current**|**Reward well**|**Replay traj.**|**Forward replay**|**Reverse replay**|**Reverse replay**|
|---|---|---|---|---|---|
|**traj.**|**visit**||**no.**|**no.**|**(%)**|
|**1**|**Center**|**1 (future)**|49|31|38.75|
|||**2 (alter. past)**|18|77|81.05|
|||**3 (alter. future)**|36|27|42.86|
|||**4 (past)**|21|72|77.42|
|**2**|**Right**|**1 (past)**|22|89|80.18|
|||**2 (future)**|88|18|16.98|
|||**3 (alter. past)**|45|34|43.04|
|||**4 (alter. future)**|61|34|35.79|
|**3**|**Center**|**1 (alter. future)**|55|24|30.38|
|||**2 (past)**|19|75|79.79|
|||**3 (future)**|62|34|35.42|
|||**4 (alter. past)**|33|100|75.19|
|**4**|**Left**|**1 (alter. past)**|25|33|56.90|
|||**2 (alter. future)**|42|28|40.00|
|||**3 (past)**|23|111|82.84|
|||**4 (future)**|81|21|20.59|



## **Table S2. (Related to Figure 3). Number of forward and reverse replay events for each behavioral condition** 

Trajectory types (i.e., trajectories 1-4) are defined as in **Figure 1A** . Only replay events during correct trials are shown. Inbound trajectories: teal; Outbound trajectories: dark blue. Alter.: alternative path. 
